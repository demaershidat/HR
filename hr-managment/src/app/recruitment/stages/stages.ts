...
export class Stages implements OnInit, OnDestroy {
 ...
 refreshData() {
  this.dataSub = forkJoin({
    ...
    this.allCandidates = candidates.map((c: any) => {
    ...
    this.uniqueCustomJobs = [...new Set(this.allCandidates.filter(c => c.custom_job).map(c => c.custom_job))];
  ...
  filterCandidates() {
    ...
    if (this.selectedFilter === 'all') {
      ...
      results = this.allCandidates.filter(c => c.custom_job === this.selectedFilter);
    } else {
      results = this.allCandidates.filter(c => Number(c.job_id) === Number(this.selectedFilter));
    }
  ...
  updateStage(candidate: any, newStageId: any) {
    ...
    const currentIdx = this.stages.findIndex(s => s.id === oldStageId);
    const newIdx = this.stages.findIndex(s => s.id === targetId);
    ...
    this.candidateService.updateCandidate(candidate.id, updatePayload).pipe(take(1)).subscribe({
      next: () => {
        ...
        const stage = this.stages.find(s => s.id === targetId);
        if (stage) {
          ...
          if (isInterview) {
            ...
            this.candidateService.createInterview(interviewData).pipe(take(1)).subscribe({
              next: () => this.router.navigate(['/recruitment/interviews']),
              error: (err) => console.error('فشل إنشاء سجل المقابلة', err)
            });
          } else if (isFinal) {
  ...
  saveStage() {
    ...
    const request = this.isEditMode && this.editingStageId
  ...
  deleteStage(stage: any) {
    if (stage.id === 1) return alert('لا يمكن حذف المرحلة الأساسية');
    if (this.allCandidates.some(c => Number(c.current_stage) === stage.id)) {
      return alert('المرحلة تحتوي على مرشحين حاليين، لا يمكن حذفها');
    }
  ...
  closeStageForm() { this.showStageForm = false; }
  ...
  getCandidatesByStage(stageId: number) {
    return this.filteredCandidates.filter(c => Number(c.current_stage) === Number(stageId));
  }

  handleImageError(event: any) { event.target.src = 'assets/unknown.png'; }
  trackByCandidateId(index: number, item: any) { return item.id; }
  trackByStageId(index: number, item: any) { return item.id; }
}