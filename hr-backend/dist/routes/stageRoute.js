...
router.post('/add', (req, res) => {
    ...
    db_1.default.query('SELECT * FROM stages WHERE name = ?', [name], (err, existing) => {
        ...
        db_1.default.query('SELECT MAX(order_index) as maxIndex FROM stages', (err, results) => {
            var _a;
            const next_index = (((_a = results[0]) === null || _a === void 0 ? void 0 : _a.maxIndex) || 0) + 1;
...
router.put('/:id', (req, res) => {
    ...
    db_1.default.query(query, [name, is_interview ? 1 : 0, is_final ? 1 : 0, id], (err, result) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.delete('/:id', (req, res) => {
    ...
    db_1.default.query('SELECT COUNT(*) as count FROM candidates WHERE current_stage = ?', [id], (err, resCandidates) => {
        if (err)
            return res.status(500).json({ error: err.message });
